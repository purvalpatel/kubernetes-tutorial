...
**Note:**
- Istio does NOT force-enable things like mTLS, tracing storage, or policies by default.
- It installs in a  "do-nothing to apps" mode.
...
```
This will show output like `attu istio-proxy`, that means envoy is deployed.

#### Quick healthcheck:
```
istioctl proxy-config endpoints attu-fcbbdc4bc-p4sjg -n milvus
```
<img width="807" height="74" alt="image" src="https://github.com/user-attachments/assets/29c8908d-b60e-445b-8ce0-c73ff2a6fefd" />

Here 2/2 means two containers are running inside one pod. <br>

#### check envoy process is running inside pod:
```
kubectl exec -n milvus attu-fcbbdc4bc-p4sjg -c istio-proxy -- ps aux
```

### If want to install observability tools:
```
...
## What Istio Enables by Default
### ✅ Automatically ON
- Envoy sidecar injection (only where you enable it)
- Traffic interception
- Telemetry generation
- Basic metrics
- Ingress Gateway (if profile includes it)
...
Istio is installed but **passive**.

### Verify Telemetry is being generated or not ?
```
kubectl exec -it attu-fcbbdc4bc-p4sjg -n milvus -c istio-proxy -- curl localhost:15090/stats/prometheus | head
```
### Check Envoy admin endpoint
```
kubectl exec -it attu-fcbbdc4bc-p4sjg -n milvus -c istio-proxy -- curl localhost:15000/stats
```


Kiali Graph:
-------------
- X-ray version of istio
- Kiali grapgh shows how servers  talk to each other inside service mesh with read traffic.
- It visualize not collects data.
...
You can check by hitting service continuosly:
```
while true; do curl http://<service>; sleep 1; done
```
